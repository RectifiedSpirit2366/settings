...
from info import ADMINS, AUTH_CHANNEL, FILE_STORE_CHANNEL, PUBLIC_FILE_STORE, DB_CHANNEL
...
async def gen_link_batch(bot, message):
    ...
    if chat_id == AUTH_CHANNEL:
        ...
        return await sts.edit(f"<code>D-{b_64}</code>")
    ...
    post = await bot.send_document(BATCH_STORE, f"batchmode_{message.from_user.id}.json", file_name="Batch.json", caption="⚠️Generated for filestore.")
    os.remove(f"batchmode_{message.from_user.id}.json")
    file_id, ref = unpack_new_file_id(post.document.file_id)
    await sts.edit(f"<code>B-{file_id}</code>")
